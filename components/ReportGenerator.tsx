...
const StepCard: React.FC<{ title: string; stepNumber: number; children: React.ReactNode }> = ({ title, stepNumber, children }) => (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-lg">
        <header className="p-4 border-b border-gray-200 flex items-center gap-4 bg-gray-50/50 rounded-t-2xl">
            <div className="w-10 h-10 flex-shrink-0 bg-gray-800 text-white rounded-full flex items-center justify-center font-bold text-base">{stepNumber}</div>
...
const ReportGenerator: React.FC<ReportGeneratorProps> = ({
    ...
    console.log('ReportGenerator component rendering with params:', params);
    ...
    const inputStyles = "w-full p-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none transition-all duration-200 placeholder:text-gray-400 text-gray-900 shadow-sm text-sm";
                    ...
                    switch (step.id) {
                        case 1: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Set up your profile and define the high-level scope of your intelligence report.</p>
                                <ProfileStep params={params} handleChange={handleChange} inputStyles={inputStyles} labelStyles={labelStyles} />
                            </StepCard>
                        );
                        case 2: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Define your market opportunity and geographic focus.</p>
                        ...
                        case 3: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Define your ideal partner and core strategic objectives.</p>
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-2">Specificity enables precise matching algorithms.</p>
                                </div>
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-2">Clear objectives enable targeted, actionable intelligence.</p>
                                </div>
                            </StepCard>
                        );
                        case 4: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Configure your analysis frameworks and AI intelligence settings.</p>
                                <div>
                                    <h4 className="text-base font-bold text-gray-900 mb-2">Analysis Tiers (Methodology)</h4>
                                    <p className="text-gray-600 text-sm mb-4">Select one or more strategic frameworks for your report. The AI will synthesize the outputs into a single, cohesive intelligence blueprint.</p>
                                    <div className="grid md:grid-cols-2 gap-4">
                                        {currentTiers.length > 0 ? currentTiers.map((tier) => (
                                            <label key={tier.id} className={`p-4 rounded-xl text-left border-2 transition-all duration-300 w-full flex flex-col h-full cursor-pointer bg-white hover:bg-gray-50 shadow-sm hover:shadow-lg ${params.tier.includes(tier.id) ? 'border-blue-500 scale-105 shadow-xl ring-2 ring-blue-500/20' : 'border-gray-200 hover:border-gray-400'}`}>
                                                <div className="flex justify-between items-start mb-2">
                                                    <span className="font-bold text-gray-900 text-lg">{tier.title}</span>
                                                    <input type="checkbox" checked={params.tier.includes(tier.id)} onChange={() => handleMultiSelectToggle('tier', tier.id)} className="h-5 w-5 rounded border-gray-300 bg-gray-100 text-blue-600 focus:ring-blue-500 focus:ring-2" />
                                                </div>
                                                <p className="text-sm text-gray-600 mb-4 flex-grow">{tier.desc}</p>
                                                <div className="border-t border-gray-200 pt-3">
                                                    <p className="text-xs font-bold text-gray-800 mb-2 uppercase tracking-wide">Capabilities</p>
                                                    <ul className="text-xs text-gray-600 space-y-1">
                                                        {tier.features.map(f => <li key={f} className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-blue-500 rounded-full"></span> {f}</li>)}
                                                    </ul>
                                                </div>
                                            </label>
                                        )) : (
                                            <div className="col-span-2 p-6 text-center bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl">
                                                <p className="text-gray-600">No tiers available for the selected organization type.</p>
                                                <p className="text-xs text-gray-500 mt-2">Please select a different organization type or contact support.</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                <div className="pt-6 border-t border-gray-200">
                                    ...
                                    <p className="text-gray-600 text-sm mt-2 mb-4">Select one or more AI personas. The AI will synthesize their expertise to provide a multi-faceted analysis.</p>
                                    <div className="grid grid-cols-4 gap-4">
                                        {AI_PERSONAS.map((persona) => (
                                            <button key={persona.id} onClick={() => handleMultiSelectToggle('aiPersona', persona.id)} className={`p-4 rounded-xl border-2 transition-all duration-300 flex flex-col items-center justify-center text-center h-full group bg-white hover:bg-gray-50 shadow-sm hover:shadow-md ${params.aiPersona.includes(persona.id) ? 'border-blue-500 scale-105 shadow-lg ring-2 ring-blue-500/20' : 'border-gray-200 hover:border-gray-400'}`} title={persona.description}>
                                                <persona.icon className={`w-8 h-8 mb-2 transition-colors duration-200 ${params.aiPersona.includes(persona.id) ? 'text-blue-600' : 'text-gray-500 group-hover:text-gray-800'}`} />
                                                <span className="font-semibold text-gray-800 text-xs leading-tight">{persona.title}</span>
                                            </button>
                                        ))}
                                        <button onClick={() => handleMultiSelectToggle('aiPersona', 'Custom')} className={`p-4 rounded-xl border-2 transition-all duration-300 flex flex-col items-center justify-center text-center h-full group bg-white hover:bg-gray-50 shadow-sm hover:shadow-md ${params.aiPersona.includes('Custom') ? 'border-blue-500 scale-105 shadow-lg ring-2 ring-blue-500/20' : 'border-gray-200 hover:border-gray-400'}`} title="Define a custom persona">
                                            <CustomPersonaIcon className={`w-8 h-8 mb-2 transition-colors duration-200 ${params.aiPersona.includes('Custom') ? 'text-blue-600' : 'text-gray-500 group-hover:text-gray-800'}`} />
                        ...
                        case 5: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Conduct automated due diligence on potential partners to identify legal, financial, and reputational risks.</p>
                                <DueDiligenceSuite
                        ...
                        case 6: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Explore a curated dashboard of potential partners, including government, NGO, and academic entities.</p>
                                <PartnerIntelligenceDashboard params={params} />
                            </StepCard>
                        );
                        case 7: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Access communication templates, cultural preparation guides, and track relationship milestones.</p>
                                <RelationshipBuilder
                        ...
                        case 8: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Forecast growth trajectories and model the impact of strategic interventions over a defined time horizon.</p>
                        ...
                        case 9: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Analyze how different local stakeholders will perceive your project to anticipate challenges and build alignment.</p>
                        ...
                        case 10: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Assess the "human factor" of a region, including safety, political stability, and quality of life for expatriates and teams.</p>
                        ...
                        case 11: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Find and compare alternative global locations that may offer a better strategic fit for your requirements.</p>
                        ...
                        case 12: return (
                            <StepCard key={step.id} title={step.title} stepNumber={step.id}>
                                <p className="text-sm text-gray-600 -mt-4 mb-4">Review all configurations and prepare for report generation.</p>
                                <ReviewStep
                                    reportData={{
    ...
    console.log('ReportGenerator returning JSX');
    return (
        <div className="flex min-h-screen bg-gray-50 font-sans text-gray-800 rounded-2xl overflow-hidden">
            ...
            <div ref={scrollPanelRef} className="flex-grow overflow-y-auto bg-gray-50 min-h-0">
                <div className="max-w-5xl mx-auto px-4 md:px-6 flex flex-col min-h-full">
                    ...
                    <footer className="flex-shrink-0 py-4 border-t border-gray-200 bg-white/80 backdrop-blur-sm sticky bottom-0 z-10">
                        ...
                        <div className="flex justify-between items-center max-w-5xl mx-auto px-4 md:px-6">
                            {currentPage > 1 ? (
                                <button onClick={prevPage} disabled={isGenerating} className="px-6 py-2.5 bg-gray-200 text-gray-800 font-semibold rounded-lg hover:bg-gray-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Back</button>
                            ) : <div></div>}
                            ...
                            {currentPage < WIZARD_PAGES.length ? (
                                ...
                                <button onClick={handleGenerateReport} disabled={isGenerating} className="px-8 py-3 bg-gray-800 text-white font-bold rounded-lg hover:bg-gray-900 transition-colors shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            ...
            {showScroll && (
                <button
                    onClick={scrollToTop}
                    className="fixed bottom-24 right-8 bg-gray-800 text-white w-12 h-12 rounded-full flex items-center justify-center shadow-lg hover:bg-gray-900 transition-all duration-300 z-50"
